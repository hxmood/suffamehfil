...
export default function AssignParticipants() {
    ...
    const fetchData = async () => {
      try {
        ...
        const [programRes, participantsRes, teamsRes] = await Promise.all([
          ...
          axios.get(`/api/participant?team=${filters.team}&category=${filters.category}`),
          axios.get('/api/teams')
        ]);
  ...
  const isParticipantAssigned = (participantId) => {
    return program?.participants.some(p => p.participant.equals(participantId));
  };

  const isTeamAtLimit = (teamId) => {
    if (!program || program.type === 'individual') return false;
    return program.participants.filter(p => p.team?.equals(teamId)).length >= 2;
  };
  ...
  const handleAssignParticipant = async (participant) => {
    try {
      ...
      const response = await axios.post(`/api/programs/${programId}/participants`, {
      ...
      <div className="max-w-7xl mx-auto">
        ...
        <div className="flex items-center justify-between mb-8">
          ...
          <h1 className="text-3xl font-bold text-[#2E2E2E] text-center">
            Assign Participants to <span className="text-[#FF6B6B]">{program?.name}</span>
          </h1>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-3 gap-6">
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            ...
            <div className="p-4 space-y-4">
              <div>
                <label className="block text-sm font-medium text-[#2E2E2E] mb-2">Team</label>
                <select
                  name="team"
                  value={filters.team}
          ...
          <div className="bg-white rounded-xl shadow-lg overflow-hidden">
            <div className="bg-[#FFC312] p-4">
              <h2 className="text-lg font-semibold text-[#2E2E2E]">Available Participants</h2>
            </div>
            <div className="p-4">
              {loading ? (
                ...
                <div className="text-center py-8">
                  <p className="text-gray-500">No participants found matching filters</p>
                </div>
              ) : (
                  ...
                  {participants.map(participant => (
                    ...
                    >
                      <div className="flex justify-between items-center">
                        <div>
                          <p className="font-medium text-[#2E2E2E]">{participant.name}</p>
                          <div className="flex items-center mt-1 space-x-2">
                            <span className={`px-2 py-1 text-xs rounded-full ${
                              participant.team?.name === 'Team A' ? 'bg-blue-100 text-blue-800' :
                              participant.team?.name === 'Team B' ? 'bg-green-100 text-green-800' :
                        ...
                        <button
                          ...
                          disabled={isParticipantAssigned(participant._id) || 
                                    (program?.type === 'group' && isTeamAtLimit(participant.team._id))}
                          ...
                          title={
                            isParticipantAssigned(participant._id) ? 'Already assigned' :
                            (program?.type === 'group' && isTeamAtLimit(participant.team._id)) ?
                              'Team limit reached' : 'Assign participant'
                          }
                        >
                          <FiUserPlus size={18} />
                        </button>
                      ...
                      <div className="flex justify-between items-center">
                        <div>
                          ...
                          <div className="flex items-center mt-1 space-x-2">
                            {team && (
                              <span className={`px-2 py-1 text-xs rounded-full ${
                                team.name === 'Team A' ? 'bg-blue-100 text-blue-800' :
                                team.name === 'Team B' ? 'bg-green-100 text-green-800' :
                                'bg-purple-100 text-purple-800'
                              }`}>
                                {team.name}
                              </span>
