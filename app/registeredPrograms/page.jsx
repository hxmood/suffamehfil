...
const ProgramsPage = () => {
  ...
  const submitEdit = async () => {
    try {
      ...
      setAllPrograms(
        allPrograms.map((p) => (p._id === editForm._id ? res.data.program : p))
      );
                  ...
                  <tbody className="bg-white divide-y divide-gray-200">
                    {filteredPrograms.map((program) => (
                      <tr key={program._id}>
                        ...
                        <td className="px-6 py-4 whitespace-nowrap">
                          <span
                            className={`px-2 py-1 text-xs rounded-full ${
                              program.category === "bzone"
                                ? "bg-blue-100 text-blue-800"
                                : program.category === "czone"
                                ? "bg-green-100 text-green-800"
                                : program.category === "yzone"
                                ? "bg-yellow-100 text-yellow-800"
                                : "bg-gray-100 text-gray-800"
                            }`}
                          >
                            {categories.find(
                              (c) => c.value === program.category
                        ...
                        <Link
                          href={`/registeredPrograms/${program._id}/assign`}
                          className="flex items-center bg-[#12CBC4] text-white px-4 py-2 rounded-lg hover:bg-[#0fa9a3] w-fit"
                        >
